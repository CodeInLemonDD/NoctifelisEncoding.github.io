...
var ASM_CONSTS = {
  3246752: function() {return Module.webglContextAttributes.premultipliedAlpha;},  
 3246813: function() {return Module.webglContextAttributes.preserveDrawingBuffer;},  
 3246877: function() {return Module.webglContextAttributes.powerPreference;},  
 3246935: function() {Module['emscripten_get_now_backup'] = performance.now;},  
 3246990: function($0) {performance.now = function() { return $0; };},  
 3247038: function($0) {performance.now = function() { return $0; };},  
 3247086: function() {performance.now = Module['emscripten_get_now_backup'];}
};
  ...
  function _DownloadFile(fileNamePtr, base64DataPtr) {
          try {
              // 从指针获取字符串
              var fileName = UTF8ToString(fileNamePtr);
              var base64Data = UTF8ToString(base64DataPtr);
              
              console.log("开始下载文件: " + fileName + ", Base64数据长度: " + (base64Data ? base64Data.length : 0));
              ...
              // 清理Base64数据（移除可能的无效字符）
              var cleanBase64Data = base64Data.replace(/[^A-Za-z0-9+/=]/g, '');
              
              // 将Base64数据转换为Blob
              var binaryString = atob(cleanBase64Data);
              ...
              console.log("文件下载已触发: " + fileName + " (大小: " + bytes.length + " 字节)");
  ...
  function _SelectFile(gameObjectName, callbackMethod, filter) {
          try {
              console.log("WebGL文件选择器启动: " + gameObjectName + "." + callbackMethod);
              
              // 强制使用固定的GameObject名称，避免实例ID问题
              var actualGameObjectName = "FileHandler";
              // 强制使用正确的方法名称，避免Unity修改
              var actualCallbackMethod = "OnWebGLFileSelected";
              console.log("强制使用GameObject名称: " + actualGameObjectName);
              console.log("强制使用方法名称: " + actualCallbackMethod);
                  ...
                  if (file) {
                      ...
                      try {
                          ...
                          var unityInstance = window.unityInstance || 
                                             window.gameInstance || 
                                             (window.UnityLoader && window.UnityLoader.UnityInstance);
                          
                          if (unityInstance && typeof unityInstance.SendMessage === 'function') {
                              unityInstance.SendMessage(actualGameObjectName, actualCallbackMethod, fileInfoJson);
                              console.log("Unity消息发送成功: " + actualGameObjectName + "." + actualCallbackMethod);
                          } else if (typeof SendMessage === 'function') {
                              SendMessage(actualGameObjectName, actualCallbackMethod, fileInfoJson);
                              console.log("使用全局SendMessage发送成功: " + actualGameObjectName + "." + actualCallbackMethod);
                          } else {
                              console.error("无法找到可用的Unity实例或SendMessage方法");
                          }
                      } catch (sendError) {
                          console.error("Unity消息发送失败: " + sendError);
                      }
                  } else {
                      console.log("文件选择取消");
                      // 通知Unity文件选择取消
                      try {
                          ...
                          if (unityInstance && typeof unityInstance.SendMessage === 'function') {
                              unityInstance.SendMessage(actualGameObjectName, actualCallbackMethod, '');
                          }
                      } catch (cancelError) {
                          console.error("取消消息发送失败: " + cancelError);
                      }
                  }
  ...
  function _SelectFileDirect(callbackMethod, filter) {
          try {
              console.log("WebGL文件选择器启动（直接调用模式）: " + callbackMethod);
              ...
              fileInput.addEventListener('change', function(event) {
                  ...
                  if (file) {
                      ...
                      try {
                          ...
                          if (unityInstance && typeof unityInstance.SendMessage === 'function') {
                              // 强制使用正确的方法名称
                              unityInstance.SendMessage("FileHandler", actualCallbackMethod, fileInfoJson);
                              console.log("Unity消息发送成功（直接调用）: FileHandler." + actualCallbackMethod);
                          } else if (typeof SendMessage === 'function') {
                              SendMessage("FileHandler", actualCallbackMethod, fileInfoJson);
                              console.log("使用全局SendMessage发送成功（直接调用）: FileHandler." + actualCallbackMethod);
                          } else {
                              console.error("无法找到可用的Unity实例或SendMessage方法");
                          }
                      } catch (sendError) {
                      ...
                      try {
                          ...
                          if (unityInstance && typeof unityInstance.SendMessage === 'function') {
                              unityInstance.SendMessage("FileHandler", actualCallbackMethod, '');
                          }
                      } catch (cancelError) {
                          console.error("取消消息发送失败: " + cancelError);
                      }
...
var asmLibraryArg = {
  ...
  "SelectFileDirect": _SelectFileDirect,
